...
- [x] **M8-7** Drag-and-drop in from Explorer, and out to Explorer with deferred rendering.
...
- [x] **M8-15** `pytest-qt` tests for every dialog and the main window flows.
...
- [x] **M9-4** Docs: README, setup guide, **disaster-recovery runbook** (rebuild, restore, doctor), threat model, ToS notice.
- [x] **M9-5** Release workflow: tag → build → sign → GitHub Release with checksums.
...
- [x] **M10-4** Security review: token handling, key zeroization, log redaction, lockfile races, header forgery resistance.
